...
sio.on("status", msg => {
    ...
    const ramData = [byteToData(status.ram.total - status.ram.free), byteToData(status.ram.total)];
    ...
    if (enable_gpu) {
        ...
        const vmemData = [byteToData((status.gpu.memory.total - status.gpu.memory.free) * 1024 * 1024), byteToData(status.gpu.memory.total * 1024 * 1024)];
...
setInterval(() => {
    ...
    const gpu = enable_gpu ? ` | GPU: ${statusData.gpu}% | VMEM: ${statusData.vmem}%(${statusData.vmemData[0]}/${statusData.vmemData[1]})` : "";
    send(`CPU: ${statusData.cpu}% | RAM: ${statusData.ram}%(${statusData.ramData[0]} / ${statusData.ramData[1]})${gpu} | DiskIO: ${statusData.disk.idleTime}%(r: ${statusData.disk.read} / w: ${statusData.disk.write}) | ${text}`)
}, 3000)
...
function getPerf() {
    exec(`TypePerf "\\PhysicalDisk(_Total)\\% Idle Time"  "\\PhysicalDisk(_Total)\\Disk Read Bytes/sec"  "\\PhysicalDisk(_Total)\\Disk Write Bytes/sec" -s ${HOSTNAME} -SC 1`, { encoding: "binary" }, (error, stdout, stderr) => {
        if (stderr && error) return
        const res = iconvDecode(stdout);
        const parse = res.split('\r\n').filter(v => v).map(v => v.replace(/"/g, ""))[1].split(",");
